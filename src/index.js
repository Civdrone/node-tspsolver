...
exports.solveTsp = function (costMatrix, roundtrip, options) {
  ...
  if (costMatrix.some(validateCostMatrixElements)) {
  ...
  if (typeof roundtrip !== "boolean") {
    return error("roundtrip expected to be a boolean!");
  }
  ...
  if (!Number.isFinite(opts.T) || opts.T <= 0) {
    return error("Invalid opts!");
  }
  if (!Number.isFinite(opts.lambda) || opts.lambda <= 0 || opts.lambda >= 1) {
    return error("Invalid opts!");
  }

  if (Number.isInteger(opts.reheatInterval) && opts.reheatInterval <= 0) {
    return error("Invalid opts!");
  }

  if (opts.reheatInterval == null) {
